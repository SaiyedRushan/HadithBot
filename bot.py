    ...
    async def send_daily_message(self):
        ...
        try:
            ...
            for channel in channels.data:
                ...
                retryCount = 0
                while not check_hadith_exists(last_hadith_no, last_book_no, last_chapter_no):
                    retryCount += 1
                    if retryCount > 3:
                        self.logger.error(f"Hadith {last_hadith_no} not found, moving to next chapter {last_chapter_no}")
                        last_chapter_no += 1 
                        last_hadith_no = 1
                        break
                ...
                current_chapter = self.get_next_index(last_hadith_no, 50884, 1)

                save_channel_state(channel_id, current_chapter, current_name_index, last_book_no, last_chapter_no)
...
class HadithCommands(app_commands.Group):
    ...
    async def specific(self, interaction: discord.Interaction, book_no: int, chapter_no: int, hadith_no: int):
        hadith = get_hadith_by_hadith_no(hadith_no, book_no, chapter_no)
        hadith = hadith[0]
        if not hadith:
            await interaction.response.send_message(
                f"Invalid hadith number. Please choose between 1 and 50884",
                ephemeral=True
            )
            return
        await interaction.response.defer()
    ...
    @app_commands.describe(start_book_id="The book id of the hadith you want to start with")
    @app_commands.describe(start_chapter_id="The chapter id of the hadith you want to start with")
    @app_commands.describe(start_hadith_id="The hadith_id of the hadith you want to start with")
    @app_commands.describe(start_name="The number of the name you want to start with")
    async def setup(self, interaction: discord.Interaction, channel_id: str, start_book_id: int, start_chapter_id: int, start_hadith_id: int, start_name: int):
        await interaction.response.defer()
        save_channel_state(channel_id, start_hadith_id, start_name, start_book_id, start_chapter_id)
    ...
    #         next_hadith_no = self.bot.get_next_index(channel['last_hadith_no'], len(self.bot.messages))
    #         next_name_no = self.bot.get_next_index(channel['last_name_no'], len(self.bot.names))
    #         await interaction.followup.send(f"Channel ID: {c.name}, Next hadith no: {next_hadith_no}, Next name no: {next_name_no}")
